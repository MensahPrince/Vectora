...
impl Effect {
    ...
    pub(crate) fn check_clip(&self, clip: &Clip) -> PyResult<()> {
        if clip.id() != self.clip_id {
            return Err(CutlassError::new_err(
                "effect does not belong to this clip",
            ));
        }
        Ok(())
    }
    ...
    pub(crate) fn set_param(
        ...
        let clip = project.model().clip(self.clip_id).ok_or_else(|| {
            CutlassError::new_err("stale effect handle")
        })?;
        let effect = clip.effects.get(self.index).ok_or_else(|| {
            CutlassError::new_err("stale effect handle")
        })?;
    ...
    fn param_index(project: &Project, clip_id: ClipId, effect_index: usize, name: &str) -> PyResult<usize> {
        let clip = project
            .model()
            .clip(clip_id)
...
impl Effect {
    fn __getitem__(&self, py: Python, name: &str) -> PyResult<f32> {
        self.with_project(py, |project| {
            ...
            let param = spec
                .param(name)
                .ok_or_else(|| PyValueError::new_err(format!("unknown effect parameter {name:?}")))?;
            Ok(effect.sample_param(param.name, 0.0).unwrap_or(param.default))
        })
    }
    ...
    fn animate(&self, py: Python, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let dict = kwargs.ok_or_else(|| {
            PyValueError::new_err("animate() requires at least one parameter")
        })?;
            ...
            for (key, value) in dict.iter() {
                ...
                if value.is_instance_of::<PyList>() {
                    ...
                    let pairs = parse_keyframe_pairs(
                        ...
                        list,
                        default_easing,
                        |v| Ok(ParamValue::Scalar(v.extract::<f32>()?)),
                    )?;
                    ...
                    let at = at_override.ok_or_else(|| {
                        PyValueError::new_err(format!(
                            "single keyframe for {name} requires at="
                        ))
                    })?;
