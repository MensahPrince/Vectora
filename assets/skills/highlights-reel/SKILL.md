...
1. Call `describe_project`. If the user marked moments (markers exist),
   ...
   their selection.
2. For each highlight, isolate a 2–5 s segment: `split_clip` at both edges
...
3. Keep the reel moving: no kept segment should exceed ~5 s; split and
...
5. Optional energy pass: speed up flat-feeling segments with
   `set_clip_speed` at 1.25–1.5x; never speed up speech the viewer must
   follow.
